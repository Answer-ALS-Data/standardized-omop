...
# Set up logging
logging.basicConfig(
    filename="logs/medical_history--condition_occurrence.log",
    level=logging.INFO,
    format="%(asctime)s - %(levelname)s - %(message)s",
)


def medical_history_medhxdsc_to_condition_occurrence_condition_concept_id(
    source_value, usagi_mapping
):
    """Convert Medical History medhxdsc values to condition concept IDs using USAGI mapping"""
    try:
        ...
        mappings = usagi_mapping[
            ...
            & (usagi_mapping["domainId"] == "Condition")
        ]
...
def main():
    try:
        ...
        # Filter USAGI mapping to only include concepts with domainId = "Condition"
        usagi_mapping = usagi_mapping[usagi_mapping["domainId"] == "Condition"]
        ...
        for _, row in source_data.iterrows():
            ...
            # Get condition_start_date using year_to_date
            condition_start_date = year_to_date(row["medhxyr"])
            ...
            concept_mappings, has_match = (
                medical_history_medhxdsc_to_condition_occurrence_condition_concept_id(
                                ...
                                [
                                    {
                                        ...
                                        "condition_source_value": f"condition: {medhxdsc_value} | mapping_equivalence: {equivalence}",
        ...
        # Save output
        output_data.to_csv("processed_source/medical_history--condition_occurrence.csv", index=False)
