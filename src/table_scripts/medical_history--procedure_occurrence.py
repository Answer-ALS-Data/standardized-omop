...
# Set up logging
logging.basicConfig(
    filename="logs/medical_history--procedure_occurrence.log",
    level=logging.INFO,
    format="%(asctime)s - %(levelname)s - %(message)s",
)


def medical_history_medhxdsc_to_procedure_occurrence_procedure_concept_id(
    source_value, usagi_mapping
):
    """Convert Medical History medhxdsc values to procedure concept IDs using USAGI mapping"""
    try:
        ...
        mappings = usagi_mapping[
            ...
            & (usagi_mapping["domainId"] == "Procedure")
        ]
...
def main():
    try:
        ...
        # Filter USAGI mapping to only include concepts with domainId = "Procedure"
        usagi_mapping = usagi_mapping[usagi_mapping["domainId"] == "Procedure"]
        ...
        for _, row in source_data.iterrows():
            ...
            # Get procedure_date using year_to_date
            procedure_date = year_to_date(row["medhxyr"])
            ...
            concept_mappings, has_match = (
                medical_history_medhxdsc_to_procedure_occurrence_procedure_concept_id(
        ...
        check_missing_concept_ids(output_data, "procedure_concept_id")

        # Save output
        output_data.to_csv("processed_source/medical_history--procedure_occurrence.csv", index=False)
